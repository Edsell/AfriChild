        ...
        <tbody>
          @forelse($posts as $p)
            <tr>
              <td>
                @if($p->image)
                  <img src="{{ asset($p->image) }}" style="height:40px;width:60px;object-fit:cover;border-radius:8px;">
                @endif
              </td>
              ...
              <td class="text-end">
                <a class="btn btn-sm btn-outline-primary" href="{{ route('sys.posts.edit', $p) }}">Edit</a>
                <form class="d-inline" method="POST" action="{{ route('sys.posts.destroy', $p) }}"
          ...
          @empty
            <tr><td colspan="7" class="text-center py-4">No posts yet.</td></tr>
          @endforelse
        </tbody>
      </table>
    </div>
